...
//            + "    yuvColor.z = texture2D(uTexture, vec2(textureCoordinate.x * 0.5, textureCoordinate.y * 0.5 + 0.5)).r;\n"
            + "    yuvColor.yz = texture2D(uTexture, textureCoordinate).ar - vec2(0.5, 0.5);\n"
            + "\n"
...
//            + "    yuvColor += offset;\n"
    ...
    public void loadYuvDataToTexture(byte[] yuvData, int width, int height) {
        ...
        mUvTextureId = OpenGlUtils.loadTexture(GLES20.GL_LUMINANCE_ALPHA, ByteBuffer.wrap(mUvData), width / 2, height / 2, mUvTextureId);
    }
